...
No useState. No setter callbacks. No re-render coordination. Set the values and everything reacts.
...
- **Custom hooks everywhere.** Framework hooks are only available inside custom widgets.
...
- **Consistency.** Every screen follows the same data flow.
